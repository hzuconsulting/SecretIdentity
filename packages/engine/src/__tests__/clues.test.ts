...
describe('soumission valide', () => {
  ...
  it('affiche la progression aux autres, sans révéler les icônes', async () => {
    ...
    // Les mains de deux joueurs peuvent se recouper — c'est voulu (`dealHand`),
    // c'est même ce qui crée l'ambiguïté du jeu. On écarte donc les icônes
    // qu'Allan a lui aussi : les recevoir est légitime, et les compter comme
    // une fuite ferait échouer ce test une fois sur vingt, au hasard du tirage.
    const allanHand = new Set(handOf(allan));
    const chosen = handOf(host)
      .filter((iconId) => !allanHand.has(iconId))
      .slice(0, 2);

    expect(chosen, 'deux icônes absentes de la main d’Allan').toHaveLength(2);
