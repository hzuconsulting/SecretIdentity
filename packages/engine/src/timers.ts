 ...
 *
 * Toute échéance du jeu (période de grâce, transfert d'hôte, fin de phase)
 * passe par ici, sous une clé nommée. Deux raisons : on peut annuler proprement
 * quand la condition disparaît (le joueur revient), et on peut tout nettoyer
 * d'un coup quand une partie se termine — sinon un `setTimeout` orphelin garde
 * une référence sur un objet `Game` mort.
 *
 * Le registre tourne dans un navigateur : ses échéances **ne sont pas fiables
 * à elles seules**, un onglet en arrière-plan les étale ou les gèle. C'est
 * `GameEngine.tick()` qui rattrape, ici on ne fait que le chemin nominal.
 */
export class TimerRegistry {
  ...
  schedule(key: string, delayMs: number, callback: () => void): void {
    ...
    // Sous Node (tests), ne pas empêcher le process de s'arrêter.
