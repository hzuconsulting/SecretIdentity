...
import { RateLimiter } from '../socket/rateLimit';
import { TEST_GRACE_MS, TestClient, startTestServer, wait } from './helpers';
...
describe('limitation de débit', () => {
  ...
  it('refuse les rafales sans casser la connexion', async () => {
    ...
    const port = await fast.listen();
    const client = await TestClient.connect(`http://127.0.0.1:${port}`);
    ...
    try {
      ...
      // La socket est toujours vivante.
