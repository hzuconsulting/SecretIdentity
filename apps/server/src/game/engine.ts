...
import { broadcastState } from '../socket/emit';

/**
 * Machine à états.
 *
 * **Seul le serveur décide d'un changement de phase.** Le client reçoit
 * `phaseEndsAt` (un timestamp serveur) et affiche un décompte, mais il peut
 * afficher `00:00` pendant une seconde sans que rien ne se passe : la
 * transition n'arrive que quand ce module la déclenche.
 *
 * Deux déclencheurs, le premier des deux qui survient (§4.1) :
...
export class GameEngine {
  ...
  async enterPhase(
    ...
    broadcastState(this.deps.io, game, now);
  ...
  private announce(game: Game, phaseEndsAt: number | null, now: number): void {
    ...
    for (const player of game.players.values()) {
      ...
      this.deps.io.to(player.socketId).emit(SERVER_EVENTS.phaseChanged, payload);
    }
  }
