...
import { broadcastState, toastAll } from '../socket/emit';
...
export async function pauseIfNeeded(deps: EngineDeps, game: Game): Promise<boolean> {
  ...
  toastAll(deps.io, game, 'Partie en pause : il manque des joueurs.', 'warning');
  broadcastState(deps.io, game, now);
...
export async function resumeIfPossible(
  ...
  toastAll(deps.io, game, 'Tout le monde est là, on reprend !', 'success');
...
async function abandonToLobby(deps: EngineDeps, code: string): Promise<void> {
  ...
  toastAll(deps.io, game, 'Partie abandonnée : retour au salon.', 'warning');
  broadcastState(deps.io, game, now);
}
